...
import kotlinx.android.synthetic.main.list_13.*
...
class List13 : ListActivity(), AbsListView.OnScrollListener {
    ...
    private val mStatus: TextView by lazy { status }
    ...
    private inner class SlowAdapter(context: Context) : BaseAdapter() {
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup): View =
                if (convertView == null) {
                    mInflater.inflate(android.R.layout.simple_list_item_1, parent, false) as TextView
                } else {
                    convertView as TextView
                }.apply {
                    if (!mBusy) {
                        this.text = mStrings[position]
        ...
        when (scrollState) {
            OnScrollListener.SCROLL_STATE_IDLE -> {
                ...
                for (i in 0 until count - 1) {
                    (view.getChildAt(i) as TextView).apply {
                        if (tag != null) {
                            text = mStrings[first + i]
                            tag = null
                        }
                    }
                }
